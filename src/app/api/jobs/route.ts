...
export async function POST(req: NextRequest) {
  ...
  const limit = limits[contractor.plan ?? 'free'] ?? 3
  if (jobsThisMonth >= limit) {
    return NextResponse.json(
      { error: `Job limit reached (${limit}/month on ${contractor.plan} plan). Upgrade to send more.` },
      { status: 403 }
    )
  }
  ...
  // US contractors MUST provide explicit payment terms — no default assumption.
  // UK: defaults to net30 if not set (statutory default).
  ...
  const termsType: string = body.paymentTermsType ?? (country === 'UK' ? 'net30' : 'net30')
