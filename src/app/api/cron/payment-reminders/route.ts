...
import { and, eq, isNotNull, isNull, lte, gte, or } from 'drizzle-orm'
...
export async function GET(req: NextRequest) {
  ...
  for (const job of jobs) {
    ...
    try {
      ...
      const paymentTermsDays = job.paymentTermsDays ?? (country === 'UK' ? 30 : 30)
      ...
      if (country === 'UK') {
        const { buildUkReminderEmail } = require('@/lib/email/uk-reminders')
        ...
        if (stage === '7_days' || stage === '14_days') {
          const { calculateStatutoryInterest } = require('@/lib/legal/uk')
        ...
        const { buildUsReminderEmail } = require('@/lib/email/us-reminders')

        // Determine late fee if applicable and contractually agreed
        let lateFeeAmount: string | undefined
        if (job.lateFeeType && job.lateFeeType !== 'none' && job.quoteSignedAt) {
          const { calculateContractualLateFee, isLateFeeEnforceable } = require('@/lib/legal/us')
